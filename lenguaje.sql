...
/*
Funciones para validar las entradas de reserva
*/

/*
Entrada: 2 entero y un caracter
Salida: No posee
Funcionalidad: Valida que los asientos no esten ocupados
*/
DROP PROCEDURE IF EXISTS  validar_entradas_asiento;
DELIMITER //
create PROCEDURE validar_entradas_asiento (in v_vuelo int, in v_fila char(2),in v_num_asiento int)
begin
		Select * from 
        temp_entradas_asiento a 
        inner join detalle_vuelo b
        on a.tipo=b.tipo_asiento
        where a.fila =b.fila
        and b.num_asiento=v_num_asiento
        and a.num_asiento=v_num_asiento
        and b.codigo_vuelo=v_vuelo
        and a.fila=v_fila;
...
Entrada: Un entero
Salida: No posee
Funcionalidad: Inserta los pasaportes en la tabla temp_entradas_pasaporte
*/
DROP PROCEDURE IF EXISTS  insertar_pasaportes;
DELIMITER //
create PROCEDURE insertar_pasaportes (in v_pasaporte int)
begin
	insert into temp_entradas_pasaporte values(v_pasaporte,pone_edad(v_pasaporte));
